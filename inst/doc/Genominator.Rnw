...
\texttt{bindAnnp = TRUE} in order to keep the annotation as part of
...
a thing easy, but appropriate matching up the annotation and the
data.  Essentially, this function ensures that you are applying the right bit of
