...
Such as object essentially points to a specific table in a specific
...
same database as the input object, possible with a different name (in
this case \texttt{counts\_tbl}.  All functions that manipulate
databases has the arguments \texttt{overwrite} and
\texttt{deleteOriginal}.  In case \texttt{deleteOriginal} is
\texttt{TRUE}, the original table (in this case \texttt{ex\_tbl} is
deleted.  If \texttt{tablename} is \texttt{NULL} (default), the
function does an in-place modification of the table (this is
destructive).
...
\Robject{eDataRaw} (which is created when the object is instantiated)
would be out of date (it would not include the \texttt{counts}
column).  While this may seem problematic, it has not been cause for
much concern.  Remember, that creating ExpData objects are very cheap,
so if something seems to break, delete them and re-instantiate.  With
a bit of familiarity, even this should not be necessary.  In general,
we highly recommend to carry out the creating/manipulation of data in
a script separate from the analysis (creation/manipulation in general
...
unwanted modifications to the database, we will instantiate a new
\Robject{eData} object in 'read' only mode.  

<<readOnly>>= 
eData <- ExpData("my.db", tablename = "counts_tbl") 
eData 
head(eData)
@
Note the use of the constructor function \Rfunction{ExpData}, this is
a standard way to instantiate ExpData objects in a new session.

It is possible to use the normal \Rfunction{[} and \Rfunction{\$}
operators on ExpData objects (although they do not have rownames).
This is rarely necessary and note that the return objects may be
massive. 
...
Note that strand needs to have values in $\{-1,0,1\}$ and that
\texttt{chr} needs to have integer values.  When you access ``real''
annotation, there is often need for a post-process step where the
annotation gets massaged into this format.  See the additional
vignette on working with the \Rpackage{ShortRead} package for a
real-life example.
...
df2 <- data.frame(chr = sample(1:16, size = N, replace = TRUE),
                  location = sample(1:1000, size = N, replace = TRUE),
                  strand = sample(c(1L,-1L), size = N, replace = TRUE))
eData2 <- aggregateExpData(importToExpData(df2, filename = "my.db", tablename = "ex2", 
                                           overwrite = TRUE))
@ 
as well as re-doing the aggregation performed previously (with a new tablename)
<<eData1>>=
eData1 <- aggregateExpData(importToExpData(df, filename = "my.db", tablename = "ex1", 
                                           overwrite = TRUE))
@ 
...
trivial. For this reason we provide a \Rfunction{joinExpData} to bind
two tables together.
...
most sense to merge two datasets, if each dataset has only a single
...
using the import facility to store the resulting object in a
...
<<joinExample>>=
## eData1 <- aggregateExpData(importToExpData(df2, filename = "my.db", tablename = "ex1", 
##                                            overwrite = TRUE))
...
Notice that both \Robject{eData1} and \Robject{eData2} has a column
named \texttt{counts}.  We use the \texttt{fields} argument to rename
these columns in the resulting object.  Also notice how we deal with
locations that have no reads mapped to them in one experiment, but
introducing missing values.  Finally, we may join an arbitrary number
of ExpData objects.
...
In this case setting \texttt{collapse} equal to \texttt{avg},
respectively \texttt{weighted.avg} yields the exact same result, since
the two experiments have the same number of reads.
...
total number of counts (the number of reads) in column ``counts'' is
...
location columns'', whereas if the function retrieves data the default
is ``all columns''.
...
location.  The minimum number of reads is not zero because we do not
store locations with no reads associated.  
...
\item Perform operations over classes of regions (genes, intergenic
  regions, ncRNAs)
\end{itemize}
There are essentially two different strategies for this.  One can
retrieve the data as a big object and then use R functions to operate
on the data (this is the approach of \Rfunction{splitByAnnotation}),
or perform some operation on the different regions in the database
(\Rfunction{summarizeByAnnotation}).  The first approach is more
flexible, but also slower and requires more memory.  The second
approach is limited to operations that can be expressed in SQL.
...
Note that the result is a \Robject{data.frame}.  We use
\texttt{bindAnno = TRUE} in order to keep the annotation as part of
the result (this is often very useful).

The value of \texttt{fxs} are SQLite functions, see
(\myurl{www.sqlite.org/lang\_aggfunc.html}).  An important note
regarding summation is in place here.  The standard SQL function
\texttt{SUM} is often less desirable for our use cases, instead we
almost always want to use the SQLite specific function \texttt{TOTAL}.
The two functions are essentially equal except for how they handle a
sum of only missing values (in R: \texttt{sum(NA)}).  \texttt{SUM}
returns a missing value and \texttt{TOTAL} returns a zero.  This is
relevant when summarizing over a genomic region containing no reads in
one experiment, but reads in another experiment.

This next example computes, for each annotated region, the number of
reads mapping to the region, ignoring strand.  Ignoring strand might
be the right approach if the protocol does not retain strand
information (as the current standard protocol for Illumina RNA-Seq
does). 
...
Note that if two regions in the annotation overlaps, data falling in
...
There are several notes here.  Note that the return object is named
according to the rownames of the annotation data, and that the return
object does \emph{not} have an element for each annotated region, but
only for each annotated region containing some data.  When the region
is empty, that region is dropped.  We provide several convenience
functions for operating on the result of such an operation (later).
...
data in each region.  The \Rfunction{applyMapped} function makes such
a thing easy, by appropriately matching up the annotation and the
data.  Essentially, this function ensures that you are applying the
right bit of annotation to the correct chunk of data.
<<applyMapped>>=
...
return some columns, this can often be significantly
faster. 
