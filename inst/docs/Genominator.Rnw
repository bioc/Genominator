...
The requirement of numeric variables in the data is purely an
optimization. The goal in this setup is often to assign experimental
data to buckets of interest a.k.a annotation. We define annotation as
...
df <- data.frame(chr = sample(1:16, size = N, replace = TRUE),
                 ...
                 category = sample(1:3, size = N, replace = TRUE))

head(df)
eDataRaw <- importToExpData(df, filename = "my.db", tablename = "ex_tbl", 
                            overwrite = TRUE, verbose = VERBOSE, columns = colnames(df))
eData <- aggregateExpData(eDataRaw, tablename = "counts_tbl", deleteOriginal = FALSE, 
                          overwrite = TRUE, verbose = VERBOSE)
@ 

Also, you can use the ``original'' table name. 
<<>>=
eData <- aggregateExpData(eDataRaw, verbose = VERBOSE)
eData
@ 

You can ``ignore'' the extra columns when you do aggregation:
<<>>=
eDataSimple <- aggregateExpData(eDataRaw, by = c("chr", "location", "strand"), tablename = "simple", overwrite = TRUE, 
                                verbose = TRUE)
head(eDataSimple)
@
...
by instanteating it here, we also put it in 'read' only mode. 
<<>>=
...
<<>>=
reg <- getRegion(eData, 1, strand = 1, whereClause = "category = 1")
head(reg)
...
cas the return value is a list where each element is an expanded
...
\section{Plotting with \Rpackage{GenomeGraph}}
In this section we demonstrate how to visualize data using the
\Rpackage{GenomeGraphs} package. The main idea is that we want to
build a plotting function which we can use to plot regions. 
